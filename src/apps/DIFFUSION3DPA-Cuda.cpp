...
//
// Leo-optimized Base_CUDA kernel: shared memory basis cache + merged load + occupancy hint
//
// Optimizations guided by Leo root cause analysis on NVIDIA H100:
//   OPT 1: Cache basis matrices (B, G) in dedicated shared memory arrays (s_B, s_G).
//          Eliminates repeated global load stalls from basis reads.
...
__global__ void Diffusion3DPA(const Real_type* __restrict__ Basis,
  ...
  // [OPT 1] Persistent basis matrices in shared memory
  ...
  // Phase 3: Forward X-basis (reads s_B, s_G from shared memory instead of global)
  GPU_FOREACH_THREAD_DIRECT(dz, z, diff::D1D) {
    GPU_FOREACH_THREAD_DIRECT(dy, y, diff::D1D) {
  ...
  // Phase 4: Forward Y-basis (reads s_B, s_G from shared memory)
  GPU_FOREACH_THREAD_DIRECT(dz, z, diff::D1D) {
  ...
  // Phase 5: Forward Z-basis + apply D operator (reads s_B, s_G from shared memory)
  ...
  // Phase 7: Backward X-basis (reads s_B, s_G from shared memory)
  ...
  // Phase 8: Backward Y-basis (reads s_B, s_G from shared memory)
  GPU_FOREACH_THREAD_DIRECT(qz, z, diff::Q1D) {
    GPU_FOREACH_THREAD_DIRECT(dy, y, diff::D1D) {
      GPU_FOREACH_THREAD_DIRECT(dx, x, diff::D1D) {
        Real_type u = 0.0, v = 0.0, w = 0.0;
  ...
  // Phase 9: Backward Z-basis + accumulate to Y (reads s_B, s_G from shared memory)
  GPU_FOREACH_THREAD_DIRECT(dz, z, diff::D1D) {
    GPU_FOREACH_THREAD_DIRECT(dy, y, diff::D1D) {
      GPU_FOREACH_THREAD_DIRECT(dx, x, diff::D1D) {
        Real_type u = 0.0, v = 0.0, w = 0.0;
