...
// Original kernel — kept for RAJA_CUDA variant (uses ZONAL_ACCUMULATION_3D macros)
...
//      arguments (x0..x7), each requiring a load to read from memory. Since
//      x0..x7 are all offsets into the same base array x, we pass only
//      x + jp + kp (3 args instead of 10 pointer args). This eliminates 7
//      pointer load instructions and the stalls that wait for them.
...
//      from the thread index, eliminating one global load and the
...
void ZONAL_ACCUMULATION_3D::runCudaVariantImpl(VariantID vid)
{
  ...
  if ( vid == Base_CUDA ) {
    ...
    for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
      ...
      RPlaunchCudaKernel( (zonal_accumulation_3d_opt<block_size>),
