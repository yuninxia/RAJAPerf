...
// Original kernel (unchanged, used by RAJA_HIP variant)
...
//
// Leo-optimized Base_HIP kernel for AMD GPUs.
//
// Based on Leo root cause analysis (AMD MI300A):
//   - 52.5% stalls from global_load_dwordx2 (basis matrices B/G/Bt + D operator)
//   - 10.6% LDS waits
//   - 20% occupancy (58 VGPRs)
//
// Optimizations applied:
//   1. Cache basis matrices (Basis, dBasis, tBasis) in LDS -- eliminates
//      repeated global loads in Phases 2-4 (forward) and Phases 6-8 (backward)
//   2. Register prefetch of D operator values before sync barrier -- overlaps
//      global memory latency with Phase 4 completion
//   3. Occupancy hint via __launch_bounds__(block_size, 8)
//   4. Merge Phase 0+1: overlap basis matrix and X loads, save one sync
//   5. Prefetch Y before Phase 7->8 sync: overlap Y read with Phase 7 completion
//
// Phases that use basis matrices (2,3,4,6,7,8) are inlined with shared-memory
// references instead of the CPA_B/CPA_G/CPA_Bt macros (which read global memory).
// Phase 1 (load X) and Phase 5 (apply D) are adapted for register prefetch.
//
template < size_t block_size >
...
__global__ void Convection3DPA_opt(const Real_type* __restrict__ Basis,
  ...
  // [OPT 1] Basis matrices cached in LDS
  ...
  // [OPT 1+4] Merged Phase 0+1: Load basis matrices AND X concurrently.
  ...
  // Phase 2: Forward X-basis -- reads s_B, s_G from LDS instead of global
  GPU_FOREACH_THREAD(dz,z,conv::D1D)
  {
    GPU_FOREACH_THREAD(dy,y,conv::D1D)
    {
  ...
  // Phase 3: Forward Y-basis -- reads s_B, s_G from LDS
  GPU_FOREACH_THREAD(dz,z,conv::D1D)
  {
    GPU_FOREACH_THREAD(qx,x,conv::Q1D)
    {
  ...
  // Phase 4: Forward Z-basis -- reads s_B, s_G from LDS
  GPU_FOREACH_THREAD(qx,x,conv::Q1D)
  {
    GPU_FOREACH_THREAD(qy,y,conv::Q1D)
    {
  ...
  // Phase 6: Backward Z-basis -- reads s_Bt from LDS
  GPU_FOREACH_THREAD(qx,x,conv::Q1D)
  {
    GPU_FOREACH_THREAD(qy,y,conv::Q1D)
    {
  ...
  // Phase 7: Backward Y-basis -- reads s_Bt from LDS
  GPU_FOREACH_THREAD(dz,z,conv::D1D)
  {
    GPU_FOREACH_THREAD(qx,x,conv::Q1D)
    {
  ...
  // [OPT 5] Prefetch Y BEFORE Phase 7->8 sync barrier.
  // The Y global load is independent of Phase 7's LDS writes (BBDGu),
  ...
  // Phase 8: Backward X-basis + accumulate -- uses s_Bt from LDS + prefetched Y
  GPU_FOREACH_THREAD(dz,z,conv::D1D)
  {
    GPU_FOREACH_THREAD(dy,y,conv::D1D)
    {
