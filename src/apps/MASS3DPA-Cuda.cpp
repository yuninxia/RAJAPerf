...
__global__ void Mass3DPA_3DBlock(const Real_type* __restrict__ B,
  ...
  for (Index_type dy = threadIdx.y; dy < mpa::D1D; dy += blockDim.y) {
    for (Index_type dx = threadIdx.x; dx < mpa::D1D; dx += blockDim.x) {
      for (Index_type dz = threadIdx.z; dz < mpa::D1D; dz += blockDim.z) {
        Xsmem[dz][dy][dx] = MPA_X(dx, dy, dz, e);
      }
    }
    for (Index_type dx = threadIdx.x; dx < mpa::Q1D; dx += blockDim.x) {
      if (threadIdx.z == 0) {
  ...
  for (Index_type dy = threadIdx.y; dy < mpa::D1D; dy += blockDim.y) {
    for (Index_type qx = threadIdx.x; qx < mpa::Q1D; qx += blockDim.x) {
      for (Index_type dz = threadIdx.z; dz < mpa::D1D; dz += blockDim.z) {
        Real_type u = 0;
        #pragma unroll
  ...
  for (Index_type qy = threadIdx.y; qy < mpa::Q1D; qy += blockDim.y) {
    for (Index_type qx = threadIdx.x; qx < mpa::Q1D; qx += blockDim.x) {
      for (Index_type dz = threadIdx.z; dz < mpa::D1D; dz += blockDim.z) {
        Real_type u = 0;
        #pragma unroll
  ...
  for (Index_type qy = threadIdx.y; qy < mpa::Q1D; qy += blockDim.y) {
    for (Index_type qx = threadIdx.x; qx < mpa::Q1D; qx += blockDim.x) {
      for (Index_type qz = threadIdx.z; qz < mpa::Q1D; qz += blockDim.z) {
        Real_type u = 0;
        #pragma unroll
  ...
  for (Index_type d = threadIdx.y; d < mpa::D1D; d += blockDim.y) {
    for (Index_type q = threadIdx.x; q < mpa::Q1D; q += blockDim.x) {
      if (threadIdx.z == 0) {
...
void MASS3DPA::runCudaVariantImpl(VariantID vid) {
  ...
  case Base_CUDA: {
    ...
    for (RepIndex_type irep = 0; irep < run_reps; RP_REPCOUNTINC(irep)) {
      ...
      RPlaunchCudaKernel( (Mass3DPA_3DBlock<MPA_OPT_BLOCK_SIZE>),
