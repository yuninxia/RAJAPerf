...
// SLM tiled GEMM kernels (Base_CUDA optimization): cooperative tile loads
// into shared memory reduce global memory traffic by ~TILE_SZ x per GEMM.
// Each tile iteration loads TILE_SZ x TILE_SZ blocks of both input matrices into
// shared memory, then computes TILE_SZ partial products per thread from shared memory.
...
void POLYBENCH_3MM::runCudaVariantImpl(VariantID vid)
{
  ...
  if ( vid == Base_CUDA ) {
    ...
    // memory reduce global memory traffic by ~TILE_SZ x per GEMM.
